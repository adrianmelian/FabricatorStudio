"""Vendored copy of the shared Fabricator LLM tool-spec registry.

This is a deliberate copy, not an import, of
``maya_tools/framework/toolbar/ai_bridge/tool_specs.py`` from the
KinematicSolutions toolset repo (same pattern as _wire.py): fabricator-mcp
ships to PyPI as an independent package and must never depend on the
toolset's internal package layout. tests/test_spec_parity.py holds the two
files equal whenever the repo layout is present.
"""
