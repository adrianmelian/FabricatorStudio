...
# project_config_paths.configs_dir, project_scaffold.candidate_dirs) are
# called directly rather than re-invented here.
...
class ProjectSetupUI(QtWidgets.QDialog):
    ...
    def create_layout(self):
        ...
        self.configs_root_label = mindmeld_style.helper_label("")
        self.main_layout.addWidget(self.configs_root_label)
    ...
    def _build_list_page(self):
        ...
        self.apply_activate_btn = mindmeld_style.button("Apply & Activate", kind="primary")
        for b in (self.new_btn, self.duplicate_btn, self.edit_btn,
                  self.delete_btn, self.apply_activate_btn):
            btn_row.addWidget(b)
        layout.addLayout(btn_row)
        return page
    ...
    def populate(self):
        self.configs_root_label.setText(self._configs_root_text())
    ...
    def _show_issues(self, issues):
        ...
        self.save_btn.setEnabled(not any(i.severity == "error" for i in issues))
    ...
    def _configs_root_text(self):
        shared = os.environ.get("FABRICATOR_PROJECT_CONFIGS")
        root = project_config_paths.configs_dir()
        kind = "SHARED" if shared else "LOCAL"
        return f"// configs root [{kind}]: {root}"
