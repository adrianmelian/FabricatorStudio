...
- **Hover annotations** (`framework/annotations.py` + `utils/qt/hover_annotation.py`) — AnimBot-style hover popovers (title + text + gif) across toolbar/palette/properties; content resolves from `maya_tools/docs/tools/<slug>.md` front-matter (`doc` keys) or the widget's own tooltip.
